% demo1 of MasonMesh tool
% This demo create a mesh for a 20x10m masonary facade sitting on a 1m
% thick foundation
% Revision history:
...
nCorners = length(data.x_local);   % 28 now
baseCornersXYZ = [data.x_local(:), data.y_local(:), zeros(nCorners,1)];
...
baseWall = zeros(nCorners, 2);
% 1–2, 2–3, ..., 27–28
for ii = 1:nCorners-1
    baseWall(ii,1) = ii;
    baseWall(ii,2) = ii+1;
end
% 28–1 (close the polygon)
baseWall(nCorners,1) = nCorners;
baseWall(nCorners,2) = 1;


% Thickness of each wall, each wall can have different thickness  
% Thickness of each wall, each wall can have different thickness 
WallThick = 0.4*ones(length(baseWall),1);
% Height of the walls
WallHeight = 17.7;
...
% --- Apply rotation to all corner points ---

%baseCornersXYZ = baseCornersXYZ * Rz.';   % (N×3) * (3×3)
...
timberElemIndex = [1 1];
title = strcat('kk_08112',timeString,'.mat');
